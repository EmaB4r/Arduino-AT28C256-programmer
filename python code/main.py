...
def send_file_via_serial(file, serial_port, start_address):
    ...
    print(hex(address_cnt), end="   ")
    while byte:=file.read(1):
        if line_cnt == 16:
            line_cnt=0
            print("\n"+hex(address_cnt), end="   ")
        serial_port.write(address_cnt.to_bytes(2, byteorder="big")) #sends the address
...
def main():
    ...
    serial_port=serial.Serial(com_device, 115200, serial.EIGHTBITS, timeout=3)
    while(1):
